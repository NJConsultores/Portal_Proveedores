...
import com.infra.factura.dto.FeCodigoMonedaDto;

public class ImporteLetraConverter {
	...
	public ImporteLetraConverter(FeCodigoMonedaDto moneda) {
		nombreDeMoneda = moneda.getDescripcion().toUpperCase();
		claveMoneda = moneda.getClave();
		if (claveMoneda.equals("MXN") || claveMoneda.equals("MXP")) {
			claveMoneda = "MN";
		}
	}
	...
	public String getStringOfNumber(String numero) {
		...
		if (Pattern.matches("\\d{1,9},\\d{1,6}", numero)) {
			...
			String decimal = new DecimalFormat(".00").format(Double.parseDouble("0." + Num[1])); 
			parte_decimal = decimal.replaceAll("\\.", "") + "/100 " + claveMoneda + ".";
			...
			return (literal + nombreDeMoneda + " " + parte_decimal)
					.toUpperCase();
	...
	private String getMillones(String numero) { // 000 000 000
		...
		String n = "";
		if (millon.length() > 1 || Integer.parseInt(millon) > 1) {
